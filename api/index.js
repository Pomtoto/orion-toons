/* ============================================================
   أوريون · توونز — الخادم الكامل في ملف واحد (نسخة محسّنة مع ميزات إضافية)
   يعمل بدون أي اعتماديات خارجية (يعتمد على Node.js 18+ فقط)
   ============================================================ */
...
app.use(async (req, res, next) => {
  // إذا كان الطلب للصور، لا نضغطها ونمررها مباشرة
...
// --- دالة ذكية لجلب العنوان حسب لغة المستخدم ---
...
function normalize(m, preferredLangs = ['en']) {
  ...
  return {
    ...
    totalChapters: 0 // سيتم تعبئتها لاحقاً إذا أردنا
  };
}
...
app.get('/api/tags', (req, res) => {
  // نقوم بتحويل الكائن الثابت إلى قائمة بالـ IDs والترجمة
  // MangaDex IDs الثابتة للمفردات (معرفات معروفة)
  const tagMap = {
    ...
    'Isekai': 'a3c3b6f5-8c1d-4a1a-9b0e-2b3c4d5e6f7a', // تقريبي
...
app.get('/api/authors/:id', async (req, res) => {
  try {
    ...
    // ترتيب الأعمال حسب الشعبية أو التاريخ
    const works = (data.relationships || [])
    ...
    res.json({
      ...
      works: works.slice(0, 20) // حد أقصى 20 عمل
    });
  } catch (e) {
...
// 5. تفاصيل عمل معين (مع عدد الفصول)
app.get('/api/works/:id', async (req, res) => {
  try {
    ...
    // جلب الإحصائيات وعدد الفصول بالتوازي
...
app.get('/api/works/:id/chapters', async (req, res) => {
  try {
    ...
    // نجيب العدد الكلي أولاً (limit=0)
    const totalRes = await fetchJSON(`/manga/${req.params.id}/feed?limit=0`);
    const total = totalRes.total || 0;

    // نجيب الشريحة المطلوبة
    const params = new URLSearchParams({
      limit: String(limit),
      offset: String(offset),
      'order[volume]': 'desc',
      'order[chapter]': 'desc',
      'translatedLanguage[]': 'en' // نفضل الإنجليزية للقراءة
    });
...
// 8. وكيل الصور (Image Proxy) - ميزة جديدة
app.get('/api/image/:mangaId/:filename', async (req, res) => {
  try {
    const { mangaId, filename } = req.params;
    // استخراج الامتداد وحجم الصورة
    const size = req.query.size || '512';
    // اسم الملف قد يكون مع أو بدون .jpg/.png
    let file = filename;
    if (!file.includes('.')) file = `${file}.${size}.jpg`; // الافتراضي JPG
    else {
      // لو موجود الامتداد نضيف الحجم قبل الامتداد
      const parts = file.split('.');
      const ext = parts.pop();
      file = `${parts.join('.')}.${size}.${ext}`;
    }

    const imageUrl = `${UPLOADS}/covers/${mangaId}/${file}`;
    const cacheKey = `img:${mangaId}:${file}`;
    const cachedBuffer = cache.get(cacheKey);
    if (cachedBuffer) {
      res.setHeader('Content-Type', 'image/jpeg');
      res.setHeader('Cache-Control', 'public, max-age=604800'); // 7 أيام
    ...
    cache.set(cacheKey, data, 604800); // نخزن لمدة 7 أيام
...
const server = app.listen(PORT, () => {
  ...
  console.log(`- Authors: /api/authors/:id`);
});
