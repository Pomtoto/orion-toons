...
function normalize(m, preferredLangs = ['en']) {
  ...
  return {
    ...
    // التعديل الأول: تمرير الرابط عبر الوكيل (Proxy) الخاص بنا بدلاً من موقع MangaDex المباشر
    img: cover ? `/api/image/${m.id}/${cover.attributes.fileName}` : null,
    description: desc,
...
app.get('/api/image/:mangaId/:filename', async (req, res) => {
  try {
    ...
    let file = filename;
    if (!file.includes('.')) file = `${file}.${size}.jpg`; 
    else {
    ...
    // التعديل الثاني: إضافة الترويسات (Headers) هنا لخداع نظام الحماية
