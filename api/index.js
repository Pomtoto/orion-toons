...
app.get('/api/health', (_req, res) => {
  res.json({ ok: true, name: 'Orion Toons', origins: ORIGINS, ocr: isConfigured() ? 'enabled' : 'disabled' });
});
...
app.get('/api/works', async (req, res) => {
  try {
    ...
    const { works, total } = await searchWorks({ origin, title, page, limit, stats });
    res.json({ total, page, works });
